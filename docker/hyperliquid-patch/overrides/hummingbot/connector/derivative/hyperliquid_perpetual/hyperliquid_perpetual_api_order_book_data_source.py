...
class HyperliquidPerpetualAPIOrderBookDataSource(PerpetualAPIOrderBookDataSource):
    ...
    async def _connected_websocket_assistant(self) -> WSAssistant:
        ...
        record_ws_connecting(self._connector, "public")
        try:
            await ws.connect(
                ws_url=url,
            ...
            record_ws_connected(self._connector, "public")
            return ws
        except Exception as exc:
            close_code, close_reason = extract_ws_close_metadata(ws)
            record_ws_closed(self._connector, "public", exc=exc, code=close_code, reason=close_reason)
            raise
    ...
    async def _parse_order_book_diff_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        ...
        record_order_book_update(self._connector, timestamp)
