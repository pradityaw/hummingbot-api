...
from hummingbot.connector.derivative.hyperliquid_perpetual.hyperliquid_runtime_connectivity import (
    ensure_runtime_state,
    record_cancel_failure,
...
_RETRYABLE_HTTP_STATUS_RE = re.compile(
    r"(?:http(?: status)?|status(?: code)?|status_code)\D*(429|5\d\d)\b"
    r"|\b(429|5\d\d)\b.*(?:gateway|server error|service unavailable|too many|rate)"
)
_RATE_LIMIT_ERROR_FRAGMENTS = (
    "too many cumulative requests sent",
    "rate limit",
    "rate-limit",
)
_NON_RETRYABLE_ACTION_TYPES = {"order"}


def _env_int(name: str, default: int, min_value: int) -> int:
    try:
        return max(min_value, int(os.environ.get(name, default)))
...
def _env_float(name: str, default: float, min_value: float) -> float:
    try:
        return max(min_value, float(os.environ.get(name, default)))
    except (TypeError, ValueError):
        return default


def _is_dns_or_connector_exception(exc: BaseException) -> bool:
    aiohttp_connector_errors = tuple(
        error_type
        for error_type in (ClientConnectorDNSError, ClientConnectorError)
    ...
    while pending:
        ...
        if isinstance(current, socket.gaierror):
            return True
        if aiohttp_connector_errors and isinstance(current, aiohttp_connector_errors):
            return True
...
_API_POST_RETRY_ATTEMPTS = _env_int("HB_HYPERLIQUID_API_POST_RETRY_ATTEMPTS", 3, 1)
_API_POST_RETRY_BASE_DELAY = _env_float("HB_HYPERLIQUID_API_POST_RETRY_BASE_DELAY", 0.25, 0.0)
_API_POST_RETRY_MAX_DELAY = _env_float("HB_HYPERLIQUID_API_POST_RETRY_MAX_DELAY", 2.0, 0.0)
_API_POST_RETRY_JITTER_RATIO = _env_float("HB_HYPERLIQUID_API_POST_RETRY_JITTER_RATIO", 0.25, 0.0)
...
class HyperliquidPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    async def _api_post(self, path_url: str, data: Optional[Dict[str, Any]] = None, **kwargs):
        for attempt in range(1, _API_POST_RETRY_ATTEMPTS + 1):
            try:
                response = await super()._api_post(path_url=path_url, data=data, **kwargs)
                record_rest_success(self, path_url)
                return response
            except asyncio.CancelledError:
                raise
            except Exception as exc:
                record_rest_failure(self, path_url, exc)
                if (
                    attempt >= _API_POST_RETRY_ATTEMPTS
                    or not self._is_retryable_api_post_exception(exc)
                    or not self._is_retry_safe_api_post(data)
                ):
                    raise
                retry_delay = self._api_post_retry_delay(attempt)
                self.logger().warning(
                    f"Transient Hyperliquid REST failure on {path_url}; retrying in {retry_delay:.2f}s "
                    f"(attempt {attempt + 1}/{_API_POST_RETRY_ATTEMPTS}). Error: {exc}"
                )
                await self._sleep(retry_delay)

    @staticmethod
    def _is_retryable_api_post_exception(exc: Exception) -> bool:
        if _is_dns_or_connector_exception(exc):
            return True
        exc_text = f"{type(exc).__name__}: {exc}".lower()
        if any(fragment in exc_text for fragment in _RATE_LIMIT_ERROR_FRAGMENTS):
            return True
        return _RETRYABLE_HTTP_STATUS_RE.search(exc_text) is not None

    @staticmethod
    def _api_post_retry_delay(attempt: int) -> float:
        base_delay = _API_POST_RETRY_BASE_DELAY * (2 ** max(attempt - 1, 0))
        jitter = random.uniform(0, base_delay * _API_POST_RETRY_JITTER_RATIO)
        return min(_API_POST_RETRY_MAX_DELAY, base_delay + jitter)

    @staticmethod
    def _is_retry_safe_api_post(data: Optional[Dict[str, Any]]) -> bool:
        if not isinstance(data, dict):
            return True
        action_type = data.get("type")
        return action_type not in _NON_RETRYABLE_ACTION_TYPES

    @staticmethod
    def _order_failure_type(exc: Exception) -> str:
        if HyperliquidPerpetualDerivative._is_retryable_api_post_exception(exc):
            exc_text = f"{type(exc).__name__}: {exc}".lower()
            if any(fragment in exc_text for fragment in _RATE_LIMIT_ERROR_FRAGMENTS):
                return "rate_limit"
            if _is_dns_or_connector_exception(exc):
                return "dns_or_connector"
            return "transient_rest"
    ...
    @staticmethod
    def _is_successful_cancel_status(status: Any) -> bool:
        if isinstance(status, str):
            return status.lower() == "success"
        return isinstance(status, dict) and "success" in status
    ...
    async def _place_order(
        ...
        try:
            ...
            record_order_failure(self, self._order_failure_type(exc), str(exc))
            raise
