...
from hummingbot.connector.derivative.hyperliquid_perpetual.hyperliquid_runtime_connectivity import (
    ...
    extract_ws_close_metadata,
    record_user_stream_update,
    record_ws_closed,
    record_ws_connected,
    record_ws_connecting,
)
...
class HyperliquidPerpetualUserStreamDataSource(UserStreamTrackerDataSource):
    ...
    def __init__(
        ...
        ensure_runtime_state(connector)
    ...
    async def _connected_websocket_assistant(self) -> WSAssistant:
        ...
        record_ws_connecting(self._connector, "private")
        try:
            await ws.connect(
                ws_url=url,
            ...
            record_ws_connected(self._connector, "private")
            self._ping_task = safe_ensure_future(self._ping_thread(ws))
            return ws
        except Exception as exc:
            close_code, close_reason = extract_ws_close_metadata(ws)
            record_ws_closed(self._connector, "private", exc=exc, code=close_code, reason=close_reason)
            raise
    ...
    async def _on_user_stream_interruption(self, websocket_assistant: Optional[WSAssistant]):
        ...
        close_code, close_reason = extract_ws_close_metadata(websocket_assistant)
        record_ws_closed(self._connector, "private", code=close_code, reason=close_reason)
        await super()._on_user_stream_interruption(websocket_assistant=websocket_assistant)

    async def _process_event_message(self, event_message: Dict[str, Any], queue: asyncio.Queue):
        record_user_stream_update(self._connector)
